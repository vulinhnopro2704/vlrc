    ...
    <section ref={sectionRef} className='bg-secondary/30 py-20 lg:py-28 dark:bg-secondary/10'>
      <div className='container mx-auto px-4'>
        <div className='mx-auto mb-16 max-w-3xl text-center'>
          ...
          <p className='text-lg text-muted-foreground'>{t('testimonials_subtitle')}</p>
        </div>

        <div ref={statsRef} className='mx-auto mb-16 max-w-3xl'>
          <div className='grid grid-cols-3 gap-8'>
            <StatItem icon={Icons.Users} value='10,000+' label={t('users')} />
            <StatItem icon={Icons.BookOpen} value='2M+' label={t('words_learned_stat')} />
            <StatItem icon={Icons.Award} value='98%' label={t('satisfaction')} />
          </div>
        </div>
      ...
      <CardContent className='p-6'>
        <div className='mb-4 flex gap-1'>
          {Array.from({ length: review.rating }).map((_, i) => (
            <Icons.Star key={i} className='h-4 w-4 fill-accent text-accent' />
          ))}
        </div>
        <p className='mb-4 text-muted-foreground leading-relaxed'>{`"${t(review.contentKey)}"`}</p>
