    ...
    <Card className='dashboard-card border-0 shadow-lg dark:bg-card/60 dark:backdrop-blur-md dark:border dark:border-white/10'>
      ...
      <CardContent>
        <div className='relative pt-4'>
          ...
          <p className='mt-4 text-center text-sm text-muted-foreground'>{t('avg_retention')}</p>
        </div>
      </CardContent>
    </Card>
  );
      ...
      <CardHeader className='pb-2'>
        <CardTitle className='flex items-center gap-2 text-lg'>
          <div className='flex h-8 w-8 items-center justify-center rounded-lg bg-accent/10 dark:bg-accent/20'>
            <Icons.Calendar className='h-4 w-4 text-accent' />
          </div>
          {t('current_streak')}
        </CardTitle>
      </CardHeader>
        ...
        <div className='pt-4 text-center'>
          ...
          <div className='mt-4 flex justify-center gap-1'>
            {[1, 1, 1, 1, 1, 0, 1].map((active, i) => (
              <div
                key={i}
                className={`h-3 w-3 rounded-full ${active ? 'bg-accent' : 'bg-muted'}`}
              />
            ))}
          </div>
          <p className='mt-3 text-sm text-muted-foreground'>{t('longest_streak', { days: 21 })}</p>
        </div>
      </CardContent>
    </Card>
  );
        ...
        <div className='flex h-32 items-end justify-between gap-2'>
          {data.map((value, i) => (
            <div key={i} className='flex flex-1 flex-col items-center gap-2'>
              ...
              />
              <span className='text-xs text-muted-foreground'>{t(days[i])}</span>
            </div>
          ))}
        </div>
