...
const exerciseTypes: LearningManagement.ActivityType[] = ['flip', 'listen-fill', 'fill-blank', 'meaning-lookup'];
...
const LessonPage = () => {
  ...
  const handleExerciseComplete = (result: LearningManagement.ActivityResult) => {
    const exerciseKey = `${result.wordId}-${result.activityType}`;
    ...
    } else if (lesson && currentIndex < lesson.words.length - 1) {
      setCurrentIndex(currentIndex + 1);
      setCurrentExerciseTypeIndex(0);
    } else {
      // Lesson completed
      console.log('[v0] Lesson completed! All exercises done.');
    }
  };
    ...
    <AppLayout>
      <div ref={pageRef} className="flex h-screen bg-background">
        <AppSidebar
          ...
          onBackToCourse={() => navigate({ to: '/courses/1' })}
        />
