...
export const VoiceCallUI = ({
  ...
  isRecording: boolean;
  isPlaying: boolean;
  liveVisemeRef: React.MutableRefObject<number>;
    ...
    <div className="absolute top-4 right-4 z-10 flex items-center gap-2">
      ...
      <Popover>
        <PopoverTrigger asChild>
          <Button variant="ghost" size="icon" className="w-9 h-9 rounded-full bg-slate-900/70 backdrop-blur-md border border-slate-700/50 text-indigo-400 hover:text-indigo-300 hover:bg-slate-800/80">
      ...
      <Popover>
        <PopoverTrigger asChild>
          <Button variant="ghost" size="icon" className="w-9 h-9 rounded-full bg-slate-900/70 backdrop-blur-md border border-slate-700/50 text-emerald-400 hover:text-emerald-300 hover:bg-slate-800/80">
            <Icons.Sparkles size={14} />
          </Button>
        </PopoverTrigger>
        <PopoverContent className="w-80 bg-slate-900/95 border border-slate-800 text-slate-100 p-5 rounded-2xl shadow-2xl backdrop-blur-md">
          <h3 className="text-sm font-bold text-emerald-400 mb-3 flex items-center gap-1.5">
            <Icons.Sparkles size={16} className="animate-pulse" /> Nhiệm Vụ ({completedObjectivesCount}/3)
          </h3>
          <div className="space-y-2.5">
            {[0, 1, 2].map(i => {
              ...
              return (
                <div key={i} className={`flex items-start gap-2.5 p-2.5 rounded-xl border ${completed ? 'bg-emerald-950/15 border-emerald-900/30 text-emerald-100' : 'bg-slate-950/25 border-transparent text-slate-400'}`}>
  ...
  const renderRecentMessagesOverlay = () => {
    const conversationMsgs = activeSession.messages.filter(m => !m.id.startsWith('ai-intro-title-') && !m.id.startsWith('ai-intro-context-'));
    ...
    return (
      ...
      >
        <div className="flex items-center justify-between mb-1">
          ...
          <Icons.ChevronUp size={12} className="text-slate-500 group-hover:text-slate-300 transition-colors" />
        </div>
        {recentMsgs.map(msg => (
          <div key={msg.id} className={`flex items-start gap-2 ${msg.role === 'You' ? 'flex-row-reverse' : ''}`}>
            <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[8px] font-bold shrink-0 ${msg.role === 'You' ? 'bg-indigo-600/80 text-white' : 'bg-slate-700/80 text-indigo-300 border border-slate-600/50'}`}>
              {msg.role === 'You' ? 'ME' : 'AI'}
            </div>
            <div className={`text-[11px] leading-snug line-clamp-2 ${msg.role === 'You' ? 'text-right text-indigo-200/80' : 'text-slate-300/80'}`}>
              {msg.text}
            </div>
          </div>
        ))}
    ...
    return (
      <div className="absolute inset-0 bg-slate-950/90 backdrop-blur-lg z-30 flex flex-col items-center justify-center p-8 text-center animate-fade-in">
        ...
        <div className="flex gap-4">
          <Button onClick={() => setActiveSession(prev => prev ? { ...prev, scenarioCompleted: false } : null)} variant="outline" className="bg-slate-900 border-slate-800 text-slate-300 rounded-full px-6">
            Xem lại cuộc đối thoại
          </Button>
          <Button onClick={() => { stopPlayback(); setActiveSession(null); }} className="bg-gradient-to-r from-emerald-500 to-teal-600 text-white rounded-full px-8 transition-all">
            Quay lại kịch bản
          </Button>
        </div>
      </div>
  ...
  return (
    <div className="relative w-full h-full flex flex-col rounded-2xl overflow-hidden border border-slate-800/60 shadow-2xl bg-slate-950">
      ...
      <div className="absolute bottom-2 left-4 z-10 text-[9px] text-slate-600 font-medium">
        Voice by <a href="https://elevenlabs.io" target="_blank" rel="noopener noreferrer" className="text-slate-500 hover:text-slate-300 underline">elevenlabs.io</a>
      </div>
      ...
      <Dialog open={showChatPopup} onOpenChange={setShowChatPopup}>
        <DialogContent className="bg-slate-950/95 border border-slate-800 text-slate-100 rounded-2xl shadow-2xl backdrop-blur-xl max-w-lg w-[90vw] h-[80vh] flex flex-col p-0">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {activeSession.messages.filter(m => !m.id.startsWith('ai-intro')).map(msg => (
              <div key={msg.id} className={`flex items-start gap-3 max-w-[85%] ${msg.role === 'You' ? 'ml-auto flex-row-reverse' : ''}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 ${msg.role === 'You' ? 'bg-indigo-600 text-white' : 'bg-slate-800 border border-slate-700 text-indigo-300'}`}>
                  {msg.role === 'You' ? 'ME' : 'AI'}
                </div>
                <div className={`p-3 rounded-xl text-xs leading-relaxed ${msg.role === 'You' ? 'bg-indigo-600 text-white rounded-tr-none' : 'bg-slate-800 text-slate-200 rounded-tl-none'}`}>
                  {msg.text}
                </div>
              </div>
            ))}
