...
pnpm preview
```
*Mở Chrome, bạn sẽ thấy biểu tượng tải app (Install App) xuất hiện ở thanh địa chỉ.*
...
- **Tạo bản APK Debug có ký tự động** (Dùng để cài trực tiếp lên điện thoại test nhanh):
  ...
  ```
  *File xuất ra tại: `android/app/build/outputs/apk/debug/app-debug.apk`*

- **Tạo bản Release chính thức** (Để ký số keystore hoặc upload lên Google Play):
  ```bash
  npx cap open android
  ```
  *Lệnh này mở dự án trong Android Studio. Tại đây, chọn **Build** > **Generate Signed Bundle / APK**.*
